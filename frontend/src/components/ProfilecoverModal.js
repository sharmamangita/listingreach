...
class ProfilecoverModal extends React.Component {
	constructor(props, context) {
		...
		this.state = {
			...
			restoreFocus:true,
			submitted: false,
			cover:'/public/assets/images/dummy-profile-pic.png' 
		};
	...
	onClickHandler(){
		...
		axios.post(`${config.uploadapiUrl}/coverupload/`,data,configs)
		.then((response) => {
			console.log(response);
			console.log("The file is successfully uploaded====",JSON.stringify(response.data.profilePic));
			this.setState({ updateprofile:response.profileimg});	
			this.setState({loadercover:false})
	...
	onClickDelete(){
		...
		dispatch(userActions.deleteprofileCover(userid.id));
		window.location.reload();
	}
	...
	render() {
		...
		if(profileuser.profileCover){
			var coverurl=`${config.uploadapiUrl}/uploads/${profileuser.profileCover}`;
		} 
			...
			<div>
				<Modal {...this.props} >
					<div className="modal-header">
						<h4 className="modal-title">Change Cover Image</h4>
					...
					<form onSubmit={this.handleSubmit} encType="multipart/form-data"  className="p-3 bg-white">
						<div className="modal-body">
							<div className="col-sm-12 text-center">
								<Spinner size={120} spinnerColor={"#333"} spinnerWidth={2} visible={this.state.loadercover} />
								<img src={coverurl?coverurl:'/public/assets/images/img_1.jpg'} style={imgstyle}/>
							</div>
							<div className="row form-group">
								<div className="col-md-12 col-lg-12 mb-3 mb-md-0">
									<label className="font-weight-bold" htmlFor="fullname">Upload Cover Image*</label>
									<div className={'form-group mb-3' + (submitted && !user.profilecover ? ' has-error' : '')}>

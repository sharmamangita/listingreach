...
class AdminUserController implements IBaseController<AdminUserBusiness> {
	...
	async getSendiBlueFolders(req: express.Request, res: express.Response) {
		var sendinBusiness: SendInBlueBusiness = new SendInBlueBusiness();
		var folders = await sendinBusiness.getAllFolfers();
		var lists = await sendinBusiness.getAllLists();
		res.send({ associations: folders, segments: lists });
	}
	async getSendInBlueLists(req: express.Request, res: express.Response) {
		let folderId = req.params["folderId"];
		var sendinBusiness: SendInBlueBusiness = new SendInBlueBusiness();
		res.send({ associations: await sendinBusiness.getAllLists() });
	}
